"""
batch_process_vision.py
=======================
Batch-process a folder of check images using Claude Vision API.

Usage:
  python batch_process_vision.py --folder ./images --output results.csv
  python batch_process_vision.py --folder ./images --output results.json
"""
...
from check_extractor_vision import extract_with_claude, validate_result

SUPPORTED_EXTENSIONS = {".jpg", ".jpeg", ".png", ".tif", ".tiff", ".bmp"}

# Anthropic API rate limit — add a small delay between requests
...
def process_folder(folder: str, output: str):
    ...
    for i, img_path in enumerate(images, 1):
        ...
        try:
            data = extract_with_claude(str(img_path))
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch-process check images using Claude Vision API."
    )
